    ...
    try:
        with sync_playwright() as p:
            ...
            browser = p.chromium.launch(
                headless=True,
                args=[
                    '--no-sandbox',                       # Cần thiết trong môi trường container
                    '--disable-gpu',                      # Tắt GPU
                    '--disable-setuid-sandbox',
                    # Giảm sử dụng /dev/shm (rất quan trọng)
            ...
            example_items = example_container.find_all(
                'div', class_='bg-inverse mb-12')

            for item in example_items:
                # --- 1. Câu Tiếng Trung (e) ---
                cn_div = item.find('div', class_='font-18 fw-400 cl-pr-sm')
                chinese_sentence_wrap = cn_div.find(
                    'span', class_='simple-tradition-wrap') if cn_div else None
                ...
                sentence_pinyin = pinyin_div.get_text(
                    strip=True) if pinyin_div else ""

                # --- 3. Bản Dịch tiếng Việt (m) ---
                viet_container = item.find(
                    'div', class_='flex-center space-between font-16 fw-400 cl-pr-sm')
                viet_translation = viet_container.find(
                    'div') if viet_container else None
                sentence_vi = viet_translation.get_text(
                    strip=True) if viet_translation else ""
